...
- **Software pipelining**: The most important optimization for inner
  ...
  iteration N execute while iterations N-1, ..., N-NS+1 are still
...
When the public API receives raw `dtype*` pointers, cast them to the
properly qualified types at the call site and pass them into a separate
core function:
...
__attribute__((noinline)) void foo_core(
    ...
    // All computation here -- compiler sees full restrict + bank info
}
```
...
### What
AIE data memory is organized into banks. Annotating pointers with their
bank assignment enables simultaneous memory operations in the same VLIW
bundle.
...
| Output (OFM)| B    | Stores via store port B, parallel with A loads   |
...
If two pointers that are loaded in the same loop iteration are
assigned to the same bank, the hardware cannot issue both loads in
the same cycle. This serializes the loads and inflates II.
...
## 4. Sub-32-Bit Load/Store Limitations

Loads and stores narrower than 32 bits (e.g., `int8`, `int16`,
`bfloat16` scalars) are hardware-limited on AIE. The memory hardware
resolves sub-word accesses by performing a full-width read-modify-write
internally. This has two consequences:
...
- **Alias analysis**: The read-modify-write destroys alias information
  ...
  the addressed byte(s). The compiler must treat sub-word accesses
  conservatively, limiting reordering and pipelining opportunities.
- **Pipelining**: Sub-word stores are particularly harmful in
  pipelined loops. The read-modify-write sequence occupies multiple
  pipeline stages, inflating II and reducing throughput. A single
  sub-word store in a loop body can prevent the pipeliner from
  achieving the resource-limited MII.

**Rule**: Always use vector-width loads and stores (`aie::load_v`,
`store_v`) in performance-critical loops. Pack data into vectors
before storing and unpack after loading. Avoid scalar sub-word
memory operations inside hot loops.

```cpp
// BAD: scalar int8 stores inside loop -- hardware-limited
for (int i = 0; i < N; i++) {
    ofm[i] = (int8_t)result;  // sub-word store each iteration
}

// GOOD: vector store -- full-width, no read-modify-write
aie::vector<int8_t, 32> result_vec = ...;
store_v(ofm_ptr, result_vec);
ofm_ptr = byte_incr(ofm_ptr, 32);
```

## 5. Loop Pragmas Reference

All macros are defined in `utils.h`. We will provide them shortly in this repo.

### Pragma table

The table below lists all `#pragma clang loop` options supported by
Peano, and the corresponding `AIE_*` wrapper macros where available.

| `#pragma clang loop` option        | AIE_* macro                       | Purpose                                                         |
|------------------------------------|-----------------------------------|-----------------------------------------------------------------|
| `min_iteration_count(N)`           | `AIE_LOOP_MIN_ITERATION_COUNT(N)` | Minimum trip count; used by the pipeliner and unroller           |
| `max_iteration_count(N)`           | `AIE_LOOP_MAX_ITERATION_COUNT(N)` | Maximum trip count; enables bounded-count optimizations          |
| (both combined)                    | `AIE_LOOP_RANGE(min,)` / `AIE_LOOP_RANGE(min, max)` | Convenience macro combining min and optional max         |
| `pipeline(disable)`                | `AIE_PREPARE_FOR_POSTPIPELINING`  | Disable the pre-RA pipeliner; the post-RA pipeliner takes over (see [Section 7](#7-software-pipelining))  |
| `pipeline_initiation_interval(N)`  | `AIE_TRY_INITIATION_INTERVAL(N)`  | Suggest a target II to the pipeliner                            |
| `unroll(disable)`                  | `AIE_LOOP_NO_UNROLL`              | Prevent loop unrolling                                          |
| `unroll(full)`                     | `AIE_LOOP_UNROLL_FULL`            | Fully unroll the loop                                           |
| `unroll_count(N)`                  | `AIE_LOOP_UNROLL(N)`              | Unroll by factor N                                              |
| `hint(key, value)`                 | `AIE_LOOP_HINT(key, value)`       | Pass arbitrary per-loop metadata to specific compiler passes    |

The pragmas that matter most for kernel performance are `pipeline(disable)`,
`pipeline_initiation_interval`, `min_iteration_count`,
`max_iteration_count`, and `hint`.

### Placement
Pragmas go immediately before the `for` statement:
...
For debugging, pass `--aie-disable-loop-option-overrides=true` to `llc`
...
- You want to guarantee the post-RA pipeliner handles the loop
- The pre-RA pipeliner produces a schedule with excessive spills
- The loop body has high register pressure
- The pre-RA pipeliner picks a poor II (check optimization remarks)
- As a general default for complex inner loops -- the post-RA
  pipeliner often produces better results because it works with
  the actual register allocation
...
determined by:
- **Resource MII**: How many cycles are needed to avoid resource
  conflicts (e.g., if the loop body needs 4 loads and only 2 load
  ports, MII >= 2)
...
The pipeliner needs to know the minimum trip count to decide:
1. Whether pipelining is profitable
2. Whether the prologue/epilogue needs guards
...
The function containing the hot loop should be `NOINLINE`
(`__attribute__((noinline))`). This ensures the compiler sees a clean
function with the loop as the dominant structure, without inlined
caller code polluting the schedule.

The public API function should be `INLINE`, doing only pointer casting
before calling the core:

```cpp
// Core: noinline, compiler focuses on loop scheduling
template <uint8_t batch_size, typename dtype>
NOINLINE void foo_core(
    dtype IFM_DM_BANK *__restrict input_ptr,
    dtype OFM_DM_BANK *__restrict output_ptr,
    params_t &params) {
    // Loop body here
}

// Wrapper: inline, casts pointers and forwards
template <uint8_t batch_size, typename dtype>
INLINE void foo(
    dtype *__restrict ifm_ptr,
    dtype *__restrict ofm_ptr,
    params_t &params) {
    foo_core<batch_size, dtype>(
        (dtype IFM_DM_BANK *)ifm_ptr,
        (dtype OFM_DM_BANK *)ofm_ptr,
        params);
}
```
...
// LESS OPTIMAL: indexed access -- requires separate address computation
...
Peano (clang-based) may need explicit template arguments where type
deduction is ambiguous:

```cpp
// May fail to compile on Peano
auto result = aie::concat(std::make_tuple(
    shuffle(v0, v1, T16_4x16_lo),
    shuffle(v0, v1, T16_4x16_hi)));

// Explicit types: compiles cleanly
